...
namespace vkInit {
	...
	SwapChainSupportDetails query_swapchain_support(const vk::PhysicalDevice& device, const vk::SurfaceKHR& surface, bool debug) {
	...
	SwapChainBundle create_swapchain(const vk::Device& logicalDevice, const vk::PhysicalDevice& physicalDevice, const vk::SurfaceKHR& surface, const int& width, const int& height, const bool& debug) {
		auto support = query_swapchain_support(physicalDevice, surface, debug);
		...
		for (const auto& image : images) {
			...
			bundle.frames.at(i).imageView = logicalDevice.createImageView(createInfo);
			i++;
